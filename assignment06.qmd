...
Where $P(W|S)$ is the probability that a marble is white given that it is small - what we are trying to find -, $P(W∩S)$ is the probability that a marble is white AND small, and $P(S)$ is the probability that a marble is small. I evaluate this for white given big to see which probability is larger, this becomes my intuitive prediction for guessing the marble color based on size. I then repeat this for black marbles.
...
The code below creates a function to generate confusion matrices and return accuracy. It takes two arguments, actual observed values, and predicted values, such as those determined above by the color_predict() function. The accuracy is .76 and the confusion matrix, formatted as a table, can be seen below. 
...
The results are the CART model are consistent with my intuitive/mental model from part 2. The accuracy is the same as well. The intuition that larger marbles will most likely be black and smaller marbles will most likely be white is supported by the CART model results; marble size is the best, and in this case basically only, predictor of marble color. The predictions in both part 2 and part 5 are the same. The reason that the predictions are the same is that size is the best available predictor that we have of marble color. So we could identify through plots that size can help to predict color, and we confirmed this through our machine learning model. In this simple case, we can see through plots and simple counts that size is such a strong predictor of color, and our CART model confirms our intuitive model. For more complex applications, the CART model should become a far better method of generating predictions than simply looking at plots and counts.